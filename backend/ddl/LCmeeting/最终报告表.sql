...
CREATE TABLE `lc_fund_performance_rating` (
    ...
    `aum_usd_mn` DECIMAL(14,2) COMMENT '资管规模(百万美元)',
    `aum_vp_pct` DECIMAL(10,4) COMMENT '占VP总AUM百分比 (如27.9%存为0.2790)',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='基金评级与基准表现月度明细表';
...
CREATE TABLE `lc_fund_performance_summary` (
    ...
    `pct_no_of_funds` DECIMAL(10,4) COMMENT '基金数量占比 (如 67% 存为 0.6700)',
    `pct_of_aum` DECIMAL(10,4) COMMENT 'AUM占比 (如 71% 存为 0.7100)',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='基金整体表现概览表';
...
CREATE TABLE `lc_fund_performance_other_accounts` (
    ...
    `aum_usd_mn` DECIMAL(14,2) COMMENT 'AUM(百万美元)',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='其他账户AUM明细表';
...
CREATE TABLE `lc_fund_performance_quartile_contribution` (
    ...
    `q1_pct` DECIMAL(10,4) NOT NULL DEFAULT '0.0000' COMMENT '第1四分位(1st Quartile)占比',
    `q2_pct` DECIMAL(10,4) NOT NULL DEFAULT '0.0000' COMMENT '第2四分位(2nd Quartile)占比',
    `q3_pct` DECIMAL(10,4) NOT NULL DEFAULT '0.0000' COMMENT '第3四分位(3rd Quartile)占比',
    `q4_pct` DECIMAL(10,4) NOT NULL DEFAULT '0.0000' COMMENT '第4四分位(4th Quartile)占比',
    
    `top_half_summary_pct` DECIMAL(10,4) COMMENT '前1/2分位汇总占比(Q1+Q2，供前端直接展示底注)',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='基金各周期四分位排名及AUM贡献分布表';
...
CREATE TABLE `lc_fund_performance` (
    ...
    `aum_usd_mn` DECIMAL(14,2) COMMENT '资管规模(百万美元)',
    `aum_vp_pct` DECIMAL(10,4) COMMENT '资管规模占比 (如 27.9% 存为 0.2790)',
    
    -- YTD 表现
    `ytd_fund` DECIMAL(10,4) COMMENT 'YTD 基金表现',
    `ytd_bm` DECIMAL(10,4) COMMENT 'YTD 基准表现',
    `ytd_excess` DECIMAL(10,4) COMMENT 'YTD 超额收益',
    
    -- 1Y 表现
    `1y_fund` DECIMAL(10,4) COMMENT '1年 基金表现',
    `1y_bm` DECIMAL(10,4) COMMENT '1年 基准表现',
    `1y_excess` DECIMAL(10,4) COMMENT '1年 超额收益',
    
    -- 年化 3Y 表现
    `ann_3y_fund` DECIMAL(10,4) COMMENT '年化3年 基金表现',
    `ann_3y_bm` DECIMAL(10,4) COMMENT '年化3年 基准表现',
    `ann_3y_excess` DECIMAL(10,4) COMMENT '年化3年 超额收益',
    
    -- 年化 5Y 表现
    `ann_5y_fund` DECIMAL(10,4) COMMENT '年化5年 基金表现',
    `ann_5y_bm` DECIMAL(10,4) COMMENT '年化5年 基准表现',
    `ann_5y_excess` DECIMAL(10,4) COMMENT '年化5年 超额收益',
    
    -- 年化 10Y 表现
    `ann_10y_fund` DECIMAL(10,4) COMMENT '年化10年 基金表现',
    `ann_10y_bm` DECIMAL(10,4) COMMENT '年化10年 基准表现',
    `ann_10y_excess` DECIMAL(10,4) COMMENT '年化10年 超额收益',
    
    -- 年化 20Y 表现
    `ann_20y_fund` DECIMAL(10,4) COMMENT '年化20年 基金表现',
    `ann_20y_bm` DECIMAL(10,4) COMMENT '年化20年 基准表现',
    `ann_20y_excess` DECIMAL(10,4) COMMENT '年化20年 超额收益',
    
    -- 成立以来表现 (Since Inception)
    `since_inc_fund` DECIMAL(10,4) COMMENT '成立以来 基金表现',
    `since_inc_bm` DECIMAL(10,4) COMMENT '成立以来 基准表现',
    `since_inc_excess` DECIMAL(10,4) COMMENT '成立以来 超额收益',
