...
def run_pipeline(
    ...
    # 只处理配置中指定的目标 Sheet（如果 qw_target_sheets 为空则不过滤）
    if config.qw_target_sheets:
        target_sheets = [s for s in all_sheets if s in config.qw_target_sheets]
        skipped = set(all_sheets) - set(target_sheets)
        if skipped:
            log_msg = f"[pipeline] Skipping {len(skipped)} non-target sheets: {skipped}"
            import logging
